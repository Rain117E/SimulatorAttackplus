...
class MetaModelFinetune(object):
    ...
    def finetune(self, q1_images, q2_images, q1_gt_logits, q2_gt_logits, finetune_times, is_first_finetune, kmeans_res, group_num):
        '''
        ...
        for img_idx, (q1_images_tensor,q2_images_tensor, each_q1_gt_logits,each_q2_gt_logits) in enumerate(zip(q1_images,
            ...
            kmeans_Class = kmeans_res[img_idx]
            self.meta_network.load_state_dict(self.batch_weights[kmeans_Class])
            ...
            for _ in range(finetune_times):
                q1_output, q1_last_features = self.meta_network.forward(q1_images_tensor)
                q2_output, q2_last_features = self.meta_network.forward(q2_images_tensor)
                if self.use_softmax:
                ...
                optimizer.step()
            self.meta_network.eval()
            self.batch_weights[kmeans_Class] = self.meta_network.state_dict().copy()
        log.info("finetune images done")

    def predict(self, q1_images, q2_images, kmeans_res, group_num):
        '''
        ...
        for img_idx, (q1_img, q2_img) in enumerate(zip(q1_images, q2_images)):
            ...
            q1_logits, q1_last_features = self.meta_network.forward(q1_img)
            q2_logits, q2_last_features = self.meta_network.forward(q2_img)
    ...
    def predict_res_features(self, images):
        ...
        for img_idx, img in enumerate(images):
            ...
            img_logits, img_l_feature = self.meta_network.forward(img)
            imgs_output.append(img_logits)
            imgs_last_features.append(img_l_feature)
        imgs_output = torch.cat(imgs_output, 0)
        imgs_last_features = torch.cat(imgs_last_features, 0)

        return imgs_output, imgs_last_features
...
class MemoryEfficientMetaModelFinetune(object):
    ...
    def finetune(self, q1_images, q2_images, q1_gt_logits, q2_gt_logits, finetune_times, is_first_finetune, img_idx_to_batch_idx, kmeans_res, group_num):
        '''
        :param q1_images: shape of (B,T,C,H,W) where T is sequence length
        :param q2_images: shape of (B,T,C,H,W)
        :param q1_gt_logits: shape of (B, T, #class)
        ...
        for img_idx, (q1_images_tensor,q2_images_tensor, each_q1_gt_logits,each_q2_gt_logits) in enumerate(zip(q1_images,
            ...
            for _ in range(finetune_times):
                ...
                if self.need_pair_distance:
                    ...
                    distance_loss = self.mse_loss(predict_distance, target_distance)
                    tot_loss = distance_loss + 0.1 * mse_error_q1 + 0.1 * mse_error_q2
                    # tot_loss = distance_loss + 0.01 * huber_error_q1 + 0.01 * huber_error_q2
                else:
                    tot_loss = mse_error_q1 + mse_error_q2
                    # tot_loss = 0.1 * huber_error_q1 + 0.1 * huber_error_q2
                optimizer.zero_grad()
                tot_loss.backward()
                optimizer.step()
    ...
    def predict(self, q1_images, q2_images, img_idx_to_batch_idx, kmeans_res, group_num):
        '''
        :param q1_images: shape of (B,C,H,W)
        :param q2_images: shape of (B,C,H,W)
        :return:
        ...
        for img_idx, (q1_img, q2_img) in enumerate(zip(q1_images, q2_images)):
            ...
            q2_logits, q2_attention_res, q2_last_features = self.meta_network.forward(q2_img)
            q1_output.append(q1_logits)
            q2_output.append(q2_logits)
    ...
    def predict_res_features(self, images):
        ...
        for img_idx, img in enumerate(images):
            ...
            self.meta_network.eval()
            img = torch.unsqueeze(img, 0)
            img_logits, img_atten, img_l_feature = self.meta_network.forward(img)
            imgs_output.append(img_logits)
            imgs_last_features.append(img_l_feature)
        imgs_output = torch.cat(imgs_output, 0)
        imgs_last_features = torch.cat(imgs_last_features, 0)
