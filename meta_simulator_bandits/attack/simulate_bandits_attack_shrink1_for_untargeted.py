    ...
    def attack_all_images(self, args, arch, tmp_dump_path, result_dump_path):
        ...
        for data_idx, data_tuple in enumerate(self.dataset_loader):
            ...
            for step_index in range(1, args.max_queries + 1):
                ...
                est_grad2 = (0.93*est_deriv.view(-1, 1, 1, 1)  + 0.07*est_deriv_mean.view(-1, 1, 1, 1))* exp_noise

                if step_index >= args.warm_up_steps and (step_index - args.warm_up_steps) % args.meta_predict_steps == 0:

                    with torch.no_grad():
                        q1_l, q2_l, q1_attens_nu, q2_attens_nu = self.meta_finetuner.predict(q1_images, q2_images, img_idx_to_batch_idx)
                        q_attens = torch.repeat_interleave((q1_attens_nu * q2_attens_nu).unsqueeze(dim=1), repeats=3,
                                                           dim=1)
